...
import { addPlansEntry,removePlansEntry,editPlansEntry,addDayToPlanInRedux,deleteDayFromPlanInRedux,updateDayInRedux,removeExerciseFromDayInRedux} from './public_manage_workouts';
import { togglePublicWorkoutsDataSelection, clearSelectedPublicWorkoutsData } from './selected_public_workouts_data';


export const SelectedExercisesScreen = ({navigation,route}) => {
  ...
  const selectedPublicWorkoutsData = useSelector((state) => state.selectedPublicWorkoutsData);
  const toggleSelection = (index) => {
    dispatch(togglePublicWorkoutsDataSelection(index));
  };
  const removeSelectedPublicWorkoutsData = () => {
    let removedSets = 0;
    let removedExercises = 0;

    const selectedExercises = selectedPublicWorkoutsData.slice(); // Copy the selectedPublicWorkoutsData array
    ...
    const allExercisesSelected = selectedPublicWorkoutsData.length === currentDayExercises.selectedExerciseRows.length;
        ...
        [
          ...
          },
          {
            ...
            onPress: () => {
              ...
              dispatch(clearSelectedPublicWorkoutsData());
              navigation.goBack();
            },
          },
        ],
    ...
    <PageContainer>
        <ScrollView>
           ...
           onPress={handleOpenSelectDayExercise}>
                  ...
                  <FormElemeentSizeButtonView style={{width:"49%"}}> 
                    <CalendarFullSizePressableButton style={{backgroundColor:'#000'}} onPress={removeSelectedPublicWorkoutsData}
           >
                            ...
                            <View style={{flexDirection:'row'}}>
                                ...
                                style={{
                                  ...
                                  <CalendarFullSizePressableButtonText style={{
                                    color: selectedPublicWorkoutsData.includes(index) ? '#00FF00' : '#000',position:'absolute',top:-2,}}>{selectedPublicWorkoutsData.includes(index) ? '✔' : ''}</CalendarFullSizePressableButtonText>
                                </CalendarFullSizePressableButton>
                            </View>
