...
export const TrainerPredefinedMealsPlanDaysScreen = ({navigation,route}) => {
...
// });
  ...
  const [speKeyFromDBLikeSpeKeySentFromPlanMeal, setSpeKeyFromDBLikeSpeKeySentFromPlanMeal] = useState(speKeySentFromPlanMeal ? speKeySentFromPlanMeal :'');

  
  const [fullDataLikeSentDay, setFullDataLikeSentDay] = useState(Object.keys(sentDay).length > 0 ? sentDay : {});
  ...
  console.log('speKeySentFromPlanMeal pred meal sentDay',speKeySentFromPlanMeal);
  console.log('speKeyFromDBLikeSpeKeySentFromPlanMeal pred meal sentDay',speKeyFromDBLikeSpeKeySentFromPlanMeal);

  
  console.log('sentDay pred meal sentDay',sentDay);

  console.log('fullDataLikeSentDay pred meal fullDataLikeSentDay',fullDataLikeSentDay);
        ...
        if(state.isConnected){
          ...
          .then(response => {
            ...
            const parsedMalAry = JSON.parse(response?.data["getTrainerPlanDays"]?.malAry);
              console.log('parsedMalAry meal pred',parsedMalAry);
              console.log('data come from db like sentday meal pred',response?.data["getTrainerPlanDays"]);

              setFullDataLikeSentDay(response?.data["getTrainerPlanDays"]);
              console.log('speKey come from DB meal pred',response?.data["getTrainerPlanDays"]?.speKey);

              setSpeKeyFromDBLikeSpeKeySentFromPlanMeal(response?.data["getTrainerPlanDays"]?.speKey);
            /************* */
...
useEffect(() => {
  ...
  const removeTodayMeals = (publicWorkoutsPlanRowCon,fullDataLikeSentDay,item,speKeyFromDBLikeSpeKeySentFromPlanMeal) => {
    
    let newData={
      trnrId: publicWorkoutsPlanRowCon?.trnrId,
      planId:publicWorkoutsPlanRowCon?.id,
      speKey:speKeyFromDBLikeSpeKeySentFromPlanMeal,
              ...
              {(data?.length >= 1) ? (
                ...
                <View style={styles.viewContainer}>
                  ...
                  <View style={{alignItems:'center', marginVertical: -12,width:"10%",height:24}}>
                    <TouchableOpacity onPress={() => removeTodayMeals(publicWorkoutsPlanRowCon,fullDataLikeSentDay,oneMeal,speKeyFromDBLikeSpeKeySentFromPlanMeal)} style={{backgroundColor:'#fff',borderRadius:10,width:20,height:20}}>
          ...
          <ResultsHalfRowLabelView>
            ...
            <ResultsHalfRowResultPlaceView><ResultsHalfRowResultPlaceViewText>{parseFloat(totalCalories?.toFixed(4)) || 0}</ResultsHalfRowResultPlaceViewText></ResultsHalfRowResultPlaceView>
              ...
              <Spacer size="large">
                ...
                onPress={() => navigation.navigate('TrainerPredefinedAddMealToPlanDaysMealsNewStyle',{publicWorkoutsPlanRowCon:publicWorkoutsPlanRowCon,speKeySentOrNormal:speKeyFromDBLikeSpeKeySentFromPlanMeal,sentDay:fullDataLikeSentDay})}>
