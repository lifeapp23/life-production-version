...
export const TrainerManageWorkoutsScreen = ({navigation,route}) => {
...
////console.log("TrainerTraineeCameData-----",TrainerTraineeCameData);
  ...
  useFocusEffect(
    ...
    AsyncStorage.getItem("currentUser").then((user) => {
        ...
        if(state.isConnected){
          ...
          console.log("Object.keys(response?.dataexistingSubscription)?.length > 0",Object.keys(response?.data["existingSubscription"])?.length > 0);
            ...
            if(Object.keys(response?.data["existingSubscription"])?.length == 0 && storedUser.id != TrainerTraineeCameData?.trnrId){
              ...
              if(response?.data["getTraineePlans"].length > 0){
                ...
                if(storedUser.id == TrainerTraineeCameData?.trneId){
                  Alert.alert(`${t(' ')}`,`${t('Personal_trainer_did_not_assign_a_workout_plans_yet')}`);
          ...
          axios.get(`https://www.elementdevelops.com/api/get-trainer-workouts-predefined-plans-with-its-days?trainerId=${storedUser.id}`, {
            headers: {
              'Authorization': `Bearer ${res}`,
              'Content-Type': 'application/json',
            },
  ...
  const removePlansDataItem = (item) => {
    ...
    if(triainerConnected){
      axios.post(`https://www.elementdevelops.com/api/trainer-trainee-plan-deleting`, item)
      .then((response) => {
      ...
      <ScrollView >
        ...
        <Spacer size="medium">
          ...
          onPress={handleOpenCalendar}>
                ...
                <TrainerTraineeWorkoutPlansCalendarScreen isVisible={isCalendarVisible} onClose={handleCloseCalendar} onSelectDateRange={handleSelectDateRange} TrainerTraineeCameData={TrainerTraineeCameData}/>
            </InputField>
          </Spacer>
          <Spacer size="medium">
            <InputField >
    ...
    <PageContainer>
        <ScrollView>
            ...
            <FormLabelView style={{width:"100%"}}>
                ...
                <View key={onePlans.id} style={styles.viewContainer}>
                  <View style={styles.plansContainer}>
                    <View style={{width:130,}}>
                      <CalendarFullSizePressableButton style={{backgroundColor:'#000',padding:5,width:130,height: 'auto',minHeight: 49,}} onPress={()=>navigation.navigate('TrainerPlanNumber',{publicWorkoutsPlanRow:onePlans})}>
                    ...
                    <>
                      <View style={{alignItems:'center', marginVertical: 15,marginRight:10,}}>
                        <Pressable
                          ...
                          onPress={()=>navigation.navigate('TrainerAddEntryPlans',{editedPlansEntry :onePlans, isEditPlansMode:true,TrainerTraineeCameData:TrainerTraineeCameData,plansWithItsDaysSent:plansWithItsDays})}
              ...
              onPress={()=>navigation.navigate('TrainerAddEntryPlans',{TrainerTraineeCameData:TrainerTraineeCameData,plansWithItsDaysSent:plansWithItsDays})} >
              ...
              <Spacer size="medium">
                <FormElemeentSizeButtonParentView style={{marginRight:10,marginLeft:10}}>
                  <CalendarFullSizePressableButton style={{backgroundColor:'#000'}}
                    onPress={() => {
                              navigation.navigate("TrainerTraineeBeginWorkout",{publicWorkoutsPlanstable:plansDataTable});
                            }} >
