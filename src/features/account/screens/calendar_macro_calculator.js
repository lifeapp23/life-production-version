...
export const CalendarMacroCalculator = ({navigation,onAddEntry}) => {
  ...
  useFocusEffect(
    React.useCallback(() => {
      ...
      AsyncStorage.getItem("currentUser").then((user) => {
        ...
        //console.log('Macro calculator calendar user---->>>',storedUser);
        fetchCalculatorsTableCalNam(storedUser.id,"macroCal").then((calculatorsResults) => {
          //console.log('calculators Macro calendar Results-->',calculatorsResults);
    ...
    <View>
      <Calendar
        ...
        onDayPress={(day) => {
          ...
          navigation.navigate('MacroCalculator',{dayWorkoutWorkedTask:dayWorkoutWorkedTask,sentPassNewDate:presseddDateString})
          onAddEntry();
        }}
      />
      <Spacer size="large">
          ...
          {(!task)?
            ...
            <DataTable >
              ...
              <DataTable.Row>
                ...
                <DataTableCellValue numeric><DataTableCellValueText >{t(task.methds)}</DataTableCellValueText></DataTableCellValue>
              </DataTable.Row>
              <DataTable.Row>
              ...
              <DataTable.Row style={{marginTop:30}}>
                <DataTableCellKey><DataTableCellKeyText style={{marginVertical:-30,paddingBottom:30}}>{t('Workout')}</DataTableCellKeyText></DataTableCellKey>
                <DataTableCellValue numeric><DataTableCellValueText style={{marginVertical:-30,paddingBottom:30}}>{task?.workot}</DataTableCellValueText></DataTableCellValue>
              </DataTable.Row>
              <DataTable.Row>
                <DataTableCellKey><DataTableCellKeyText>{t('Target')}</DataTableCellKeyText></DataTableCellKey>
                <DataTableCellValue numeric><DataTableCellValueText>{task?.target}</DataTableCellValueText></DataTableCellValue>
              </DataTable.Row>
              <DataTable.Row style={{marginTop:30}}>
                <DataTableCellKey><DataTableCellKeyText style={{marginVertical:-30,paddingBottom:30}}>{t('Type_of_Diet')}</DataTableCellKeyText></DataTableCellKey>
                <DataTableCellValue numeric><DataTableCellValueText style={{marginVertical:-30,paddingBottom:30}}>{task?.ditTyp}</DataTableCellValueText></DataTableCellValue>
              </DataTable.Row>
              <DataTable.Row>
                <DataTableCellKey ><DataTableCellKeyText >{t('Calories')}</DataTableCellKeyText></DataTableCellKey>
                <DataTableCellValue numeric><DataTableCellValueText >{parseFloat(task?.calris).toFixed(0)} {t('kcal_day')}</DataTableCellValueText></DataTableCellValue>
              </DataTable.Row>
              <DataTable.Row>
                <DataTableCellKey ><DataTableCellKeyText >{t('Protein')}</DataTableCellKeyText></DataTableCellKey>
                <DataTableCellValue numeric><DataTableCellValueText >{parseFloat(task?.protin).toFixed(0)} {t('kcal_day')}</DataTableCellValueText></DataTableCellValue>
              </DataTable.Row>
              <DataTable.Row>
                <DataTableCellKey ><DataTableCellKeyText >{t('Fats')}</DataTableCellKeyText></DataTableCellKey>
                <DataTableCellValue numeric><DataTableCellValueText >{parseFloat(task?.fats).toFixed(0)} {t('kcal_day')}</DataTableCellValueText></DataTableCellValue>
              </DataTable.Row>
              <DataTable.Row>
                <DataTableCellKey ><DataTableCellKeyText >{t('Carbs')}</DataTableCellKeyText></DataTableCellKey>
                <DataTableCellValue numeric><DataTableCellValueText >{parseFloat(task?.carbs).toFixed(0)} {t('kcal_day')}</DataTableCellValueText></DataTableCellValue>
              </DataTable.Row>
