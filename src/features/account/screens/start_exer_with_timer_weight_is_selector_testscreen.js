...
const SecondTimer = React.memo(({ onChangeInputValues, index, publicPlansDataTableItemDayNewData, activeIndex, set, activeTimerIndex, setActiveTimerIndex }) => {
    ...
    if (activeTimerIndex === index) {
      if (isStarted) {
        intervalRef.current = setInterval(() => {
          setTimerTwo((prevTimer) => prevTimer + 1);
        }, 1000);
      } else {
...
export const TrainerTraineeStartExercisesWithTimerScreen = ({navigation,route}) => {
  const {publicWorkoutsPlanstableCon,publicPlansDataTableItemDayCon,publicWorkoutsPlanDayArrBeforeSeperateWrkArySent} = route.params;
...
useEffect(() => {
  ...
  .then((res) => { 
    AsyncStorage.getItem("currentUser").then((user) => {
      ...
      const wrkKeys = publicPlansDataTableItemDayNewData.map(item => parseInt(item.wrkKey));
      ...
      //   });
        ...
        const fetchData = async () => { 
          ...
          if(state.isConnected){
            ...
            axios.get(`https://www.elementdevelops.com/api/fetch-last-day-start-workouts`, {
              params: {
...
// //////console.log('sets.workedExercises222222-------',sets?.[publicWorkoutsPlanDayArrBeforeSeperateWrkArySent?.[0].speKey]?.workedExercises);
  ...
  const handleToggleComplete = (WrkTypTimInitSent,plnKey,speKey,dayNam,date,wrkKey,wktNam,setId, index, weight, reps,dayTim, array,casTim,exrTyp,exrTim) => {
    const activeWorkoutId = publicPlansDataTableItemDayNewData[activeIndex].wrkKey;
...
////////console.log('sets after ------------', sets[publicWorkoutsPlanDayArrBeforeSeperateWrkArySent?.[0].speKey].workedExercises);
    ...
    if ( isAllCompleted ) {
      setLastRowPressed(true);
      const combinedArray = combineArrays(updatedSets?.[publicWorkoutsPlanDayArrBeforeSeperateWrkArySent?.[0].speKey]?.workedExercises);
...
const WorkoutsInputs =({sets,publicPlansDataTableItemDay,publicPlansDataTableItemDayNewData,activeIndex,handleToggleComplete,WrkTypTimInitSent})=>{
  {/* Rows */}
...
// Loop over the exercises (if multiple)
  ...
  if (newWorkoutsFromDB?.[currentId]?.[index]?.weight !== undefined) {
    const workoutData = newWorkoutsFromDB?.[publicPlansDataTableItemDayNewData?.[i]?.wrkKey][index];
    ...
    <View key={`${row.wrkKey}-${index}`} style={[styles.row,styles.InputsRows]}>
          <Text style={[styles.cell,isArabic ? styles.ArabicSetsBoddy : styles.EnglishSetsBoddy]}>{set.sets}</Text>
            ...
            {row?.exrTyp === 'Cardio' || row?.exrTyp === 'Stability' ? (
              <SecondTimer onChangeInputValues={setInputValues} index={index} publicPlansDataTableItemDayNewData={publicPlansDataTableItemDayNewData} activeIndex={activeIndex} set={set} activeTimerIndex={activeTimerIndex} setActiveTimerIndex={setActiveTimerIndex}/>
            ) : row?.exrTyp === '' ? (
        ...
        <TouchableOpacity
          ...
          ((parseFloat(inputValues[publicPlansDataTableItemDayNewData[activeIndex].wrkKey]?.[index]?.casTim) > 0) )?(
              handleToggleComplete(WrkTypTimInitSent,set.plnKey,set.speKey,set.dayNam,set.date,set.wrkKey,set.wktNam,set.sets, index, 0, 0,set.dayTim, array,inputValues[publicPlansDataTableItemDayNewData[activeIndex].wrkKey]?.[index]?.casTim,set.exrTyp,set.exrTim)):(
  ...
  try {
    const intervalOne = setInterval(async () => {
      setTimerOne(prevTimer => {
        ...
        const  ExerciesSpekey = publicWorkoutsPlanDayArrBeforeSeperateWrkArySent?.[activeIndex]?.speKey;
        const  ExerciesWrkKey = publicWorkoutsPlanDayArrBeforeSeperateWrkArySent?.[activeIndex]?.wrkKey;
...
const subscription_handleAppStateChange = AppState.addEventListener('change', handleAppStateChange);
 ...
 const unsubscribeForIntervalOne = navigation.addListener('beforeRemove', async () => {
   ...
   // Clear the stored time in AsyncStorage
    ...
    publicWorkoutsPlanDayArrBeforeSeperateWrkArySent.forEach(async (item, index) => {
      const { speKey, wrkKey } = item;
        const key = `workout_${speKey}_${wrkKey}_${index}`;
        const savedLiveTimer = `workout_${speKey}_${wrkKey}_${index}_timer`;
...
const showTrainingStartAlert = async () => {
      ...
      <ScrollView>
        ...
        <View style={styles.container}>
          <View style={styles.table}>
            ...
            <WorkoutsInputs sets={sets} publicPlansDataTableItemDay={publicPlansDataTableItemDay} publicPlansDataTableItemDayNewData={publicPlansDataTableItemDayNewData} activeIndex={activeIndex} handleToggleComplete={handleToggleComplete} WrkTypTimInitSent={WrkTypTimInitSent} />
