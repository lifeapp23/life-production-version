...
import { useTranslation } from 'react-i18next';//add this line
  ...
  export const TrainerAddEntryPlansMealsScreen =({navigation,route})=>{
    ...
    .then((response) => {
        ...
        Alert.alert(`${t(' ')}`,`${t('Your_Plan_added_to_Database_successfully')}`,
          ...
          if (selectedPlanWithItsDays?.plan_days?.length > 0) {
             ...
             const dayObjectArray = selectedPlanWithItsDays?.plan_days?.map(meal => {
               
               let mealMalAryParsed = JSON.parse(meal?.malAry);
               const updatedmeals = mealMalAryParsed.map((mealData) => {
                 ...
                 //console.log(' mealWrkAryParsed.map imageName',imageName);
                 // //console.log('imageUri',imageUri);
                 // //console.log('imageExt',imageExt);
             
                 let mealInfo = {
                   foddes: mealData.foddes,
                   date: mealData.date,  // Using wrkKey from the original object
                   time: mealData.time,
                   ...
                   Iron: mealData.Iron,
                   images: mealData.images,  // Use imageName if it exists, otherwise set it to an empty string
                   imageId: mealData.imageId,  // Use imageName if it exists, otherwise set it to an empty string
                   imageNameNew: imageName || '',  // Use imageName if it exists, otherwise set it to an empty string
                   mealSpekey:mealData?.mealSpekey,
                   malChk:mealData?.malChk,
                   datChk:mealData?.datChk,
               ...
               const newMalAry = JSON.stringify(updatedmeals);
               return {
                 trnrId: TrainerTraineeCameData?.trnrId,
                 trneId: TrainerTraineeCameData?.trneId,
                 planId: newPlanId,
                 speKey: meal?.speKey,
                 dayNam: meal?.dayNam,
                 malAry: newMalAry, // Multiple meals per day
               };
             });
         
             const MealsDaysFormData = new FormData();
 
             dayObjectArray.forEach((dayObj, dayIndex) => {
               MealsDaysFormData.append(`meals[${dayIndex}][trnrId]`, dayObj.trnrId);
               MealsDaysFormData.append(`meals[${dayIndex}][trneId]`, dayObj.trneId);
               MealsDaysFormData.append(`meals[${dayIndex}][planId]`, dayObj.planId);
               MealsDaysFormData.append(`meals[${dayIndex}][speKey]`, dayObj.speKey);
               MealsDaysFormData.append(`meals[${dayIndex}][dayNam]`, dayObj.dayNam);
               MealsDaysFormData.append(`meals[${dayIndex}][malAry]`, dayObj.malAry);
               
               let dayObjMalAryParsed = JSON.parse(dayObj?.malAry);
 
               // Handle multiple images
               dayObjMalAryParsed.forEach((meal, index) => {
                 //console.log('dayObjMalAryParsed !meal: ',meal);
 
                 let imageUri = meal?.images;
                 //console.log('dayObjMalAryParsed !imageUri: ',imageUri);
 
                 let imageName = "";
                 let imageExt = "";
                 if (imageUri && imageUri.startsWith('file:///data/user')) {
                   imageName = imageUri.split('imeages/').pop();
                   imageExt = imageUri.split('.').pop();
                   MealsDaysFormData.append(`meals[${dayIndex}][images][${index}]`, {
                     uri: imageUri,
                     name: imageName,
                     type: `image/${imageExt}`,
                   });
             ...
             .then(response => {
              //console.log('Your_Plan_added_to_Database_successfully');

               Alert.alert(
                 `${t(' ')}`,
                 `${t("Your_Plan_added_to_Database_successfully")}`,
                 [
                   {
                     text: 'OK',
                     onPress: () => {
   ...
   if(triainerConnected){
      if(!isPredefinedPlansOn){
        ...
        if (selectedPlanWithItsDays?.plan_days?.length > 0) {
           ...
           .then(response => {
            //console.log('Your_Plan_updated_to_Database_successfully');
      ...
      <ScrollView >
            ...
            <FormLabel><AsteriskTitle>*</AsteriskTitle> {t("Predefined_plans")}:</FormLabel>
              ...
              <FormInputView style={{width:"55%"}}>
                ...
                onSelect={index => {
                  ...
                  setSelectedPlanWithItsDays(plan);
                  //console.log('Selected Plan:', plan); // This will log the full plan info when selected
        ...
        <Spacer size="medium">
          ...
          onPress={handleOpenCalendar}>
              ...
              <TrainerTraineeManageMealsCalendarScreen isVisible={isCalendarVisible} onClose={handleCloseCalendar} onDateSelect={handleDateSelect} TrainerTraineeCameData={TrainerTraineeCameData}/>
