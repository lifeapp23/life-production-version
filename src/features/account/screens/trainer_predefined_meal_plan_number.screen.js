  ...
  useFocusEffect(
    React.useCallback(() => {
      ...
      if(state.isConnected){
        ...
        axios.get(`https://www.elementdevelops.com/api/get-trainer-predefined-meal-plan-days?trainerId=${publicWorkoutsPlanRowCon?.trnrId}&planId=${publicWorkoutsPlanRowCon?.id}`, {
        headers: {
...
const newSkippedCompleted = (publicWorkoutsPlanDayArr,publicWorkoutsPlanRowCon,publicWorkoutsPlanDayArrBeforeSeperateWrkAry)=>{
  ...
  const handleRemoveView = (trnrId, speKey, planId) => {
    ...
    if(triainerConnected){
      axios.post(`https://www.elementdevelops.com/api/trainer-Predefined-meal-plan-day-deleting`, newData)
      .then((response) => {
          //console.log('deleted');
          setPublicWorkoutsPlanDaysTable(response?.data?.["getTrainerPlanDays"]);
